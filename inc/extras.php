 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'webionlite_set_post_meta_value' );

/**
 * Container type
 */
add_filter( 'theme_mod_container_type', 'webionlite_set_post_meta_value' );

/**
 * Header layout type
 */
add_filter( 'theme_mod_header_layout_type', 'webionlite_set_header_layout_value' );
 ...
 * @return string
 */
function webionlite_set_post_meta_value( $value ) {
	$queried_obj = webionlite_get_queried_obj();

	if ( ! $queried_obj ) {
		return $value;
	}

	$meta_key   = 'webionlite_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * @return string
 */
function webionlite_set_header_layout_value( $value ) {

	if ( wp_is_mobile() ) {
		return 'mobile';
	}

	return webionlite_set_post_meta_value( $value );
}
 ...
 */
function webionlite_get_queried_obj() {
	$queried_obj = apply_filters( 'webionlite-theme/posts/queried_object_id', false );

	if ( ! $queried_obj && ! webionlite_maybe_need_rewrite_mod() ) {
		return false;
	}
 ...
 * @return boolean
 */
function webionlite_maybe_need_rewrite_mod() {
 ...
 * @return string
 */
function webionlite_render_macros( $string ) {

	$macros = apply_filters( 'webionlite-theme/data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function webionlite_render_icons( $content ) {
	$icons     = webionlite_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'webionlite_render_icons_callback', $content );
}
 ...
 * @return string
 */
function webionlite_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function webionlite_get_render_icons_set() {
	return apply_filters( 'webionlite-theme/icons/icons-set', array(
 ...
 * @return string
 */
function webionlite_render_theme_url( $url ) {
 ...
 * @return string
 */
function webionlite_justify_thumbnail_size( $mask = 0, $thumbnail_size = 'post-thumbnail', $justify_size='webionlite-thumb-justify', $justify_size_1 = 'webionlite-thumb-justify', $justify_size_2 = 'webionlite-thumb-justify-2') {
	$mask_list = array(
 ...
 * @return string
 */
function webionlite_get_post_template_part_slug() {
	return apply_filters( 'webionlite-theme/posts/template-part-slug', 'template-parts/content' );
}
 ...
 * @return string
 */
function webionlite_get_post_style() {
	return apply_filters( 'webionlite-theme/posts/post-style', false );
}
 ...
 * @return array
 */
function webionlite_kses_post_allowed_html( $additional_allowed_html = array() ) {
