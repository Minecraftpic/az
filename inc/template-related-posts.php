 ...
 * @return array
 */
function webionlite_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = webionlite_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = webionlite_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = webionlite_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? webionlite_post_thumbnail( 'webionlite-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? webionlite_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? webionlite_posted_on( array( 'echo' => false ) ) : '';
